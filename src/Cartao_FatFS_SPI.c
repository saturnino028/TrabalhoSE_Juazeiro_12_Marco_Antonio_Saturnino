...
/**
 * @brief lista todas as unidades SSD encontradas
 */
void listar_cartoes_disponiveis()
{
    size_t total = sd_get_num();
    if (total == 0) {
        printf("Nenhum cartão SD encontrado.\n");
        return;
    }

    printf("Cartões SD encontrados:\n");
    for (size_t i = 0; i < total; ++i) {
        sd_card_t *pSD = sd_get_by_num(i);
        if (pSD) {
            printf("  [%zu] Nome: %s | Montado: %s\n",
                   i,
                   pSD->pcName,
                   pSD->mounted ? "Sim" : "Não");
        } else {
            printf("  [%zu] Cartão inválido.\n", i);
        }
    }
}

/**
 * @brief função para verificar se existe algum cartão pronto para uso
 */
bool cartao_pronto()
{
    size_t total = sd_get_num();
    ...
    for (size_t i = 0; i < total; ++i) {
        sd_card_t *pSD = sd_get_by_num(i);
        if (pSD && pSD->mounted) {
            printf("Cartão SD montado encontrado: %s\n", pSD->pcName);
            return true;
        }
    }

    printf("Nenhum cartão SD montado.\n");
    return false;
}